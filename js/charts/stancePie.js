...
const colors = CONFIG.colors.stanceMacro;

export function renderStancePie(container, register, state, onFilter) {
  ...
  const counts = d3.rollup(register, v => v.length, d => d.stanceMacro);
  ...
  arcs.append('path')
    .attr('d', arc)
    .attr('fill', d => colors[d.data.key] || '#64748b')
    .attr('opacity', d => !state.stanceMacro || state.stanceMacro === d.data.key ? 1 : 0.3)
    .on('mouseover', (event, d) => {
      d3.select(event.currentTarget).attr('d', arcHover);
      showTooltip(event, `<strong>${d.data.key}</strong>
        <span class="t-row"><span>Submissions</span><span class="t-val">${d.data.value}</span></span>
    ...
    .on('click', (_, d) => onFilter('stanceMacro', d.data.key));

  // Centre label
  const centreText = g.append('g');
  centreText.append('text')
    .attr('text-anchor', 'middle').attr('dy', '-0.3em')
    .attr('font-size', '22px').attr('font-weight', '700').attr('fill', '#e6edf3')
    .text(total);
  centreText.append('text')
  ...
  data.forEach(d => {
    const item = legend.append('div')
      ...
      .classed('dimmed', state.stanceMacro && state.stanceMacro !== d.key)
      .on('click', () => onFilter('stanceMacro', d.key));
    item.append('div')
      .attr('class', 'legend-swatch')
      .style('background', colors[d.key] || '#64748b');
