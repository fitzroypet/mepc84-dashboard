...
function buildLegend(container, countryList) {
  ...
  d3.select(container)
    .append('p')
    .attr('class', 'map-note')
    .text('Gray countries do not have a matching classification record in the local table.');
}
...
export async function renderWorldMap(container, countryList) {
  ...
  svg.append('g')
    ...
    .attr('fill', d => {
      ...
      return country ? getClassificationColor(country.finalClassification) : NO_DATA_FILL;
    })
    .attr('stroke', '#09111b')
    ...
    .on('mouseover', (event, d) => {
      const iso3 = NUM_TO_ISO3[String(d.id)];
      const country = iso3 ? countryByIso3.get(iso3) : null;
      const countryName = country?.country || iso3 || 'Unknown country';
